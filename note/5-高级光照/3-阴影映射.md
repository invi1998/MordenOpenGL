...
在右边的图中我们显示出同样的平行光和观察者。我们渲染一个点$\vec{P}$处的片段，需要决定它是否在阴影中。我们先得使用$T$把$\vec{P}$变换到光源的坐标空间里。既然点$\vec{P}$是从光的透视图中看到的，它的z坐标就对应于它的深度，例子中这个值是0.9。使用点$\vec{P}$在光源的坐标空间的坐标，我们可以索引深度贴图，来获得从光的视角中最近的可见深度，结果是点$\vec{C}$，最近的深度是0.4。因为索引深度贴图的结果是一个小于点$\vec{P}$的深度，我们可以断定$\vec{p}$被挡住了，它在阴影中了。
