...
1. 使用 `glGenTextures()` 函数生成一个纹理 ID，并将其绑定到当前上下文中。
2. 设置纹理参数。可以使用 `glTexParameteri()` 函数设置纹理的过滤方式、环绕方式等参数。对于深度立方体贴图，通常采用 GL_NEAREST 或 GL_LINEAR 采样方式，并通过 GL_CLAMP_TO_EDGE 环绕方式避免边界问题。
3. 分配内存空间。使用 `glTexImage2D()` 函数为纹理分配内存空间，并初始化纹理数据。由于是深度立方体贴图，因此需要分别指定每个面和每个层级的深度格式和大小。
4. 创建帧缓冲区对象（FBO）。使用 `glGenFramebuffers()` 函数生成一个 FBO，并将其绑定到当前上下文中。
5. 将深度立方体贴图附加到 FBO 中。可以使用 `glFramebufferTexture2D()` 函数将深度立方体贴图作为深度纹理附加到 FBO 的不同面上。
6. 检查 FBO 是否完整。可以使用 `glCheckFramebufferStatus()` 函数检查 FBO 是否完整并可用于渲染。
7. 渲染场景，并将深度信息渲染到深度立方体贴图中。在渲染过程中，需要使用 FBO 激活深度立方体贴图，并从六个方向分别渲染场景深度信息到相应的纹理面上。
8. 取消 FBO 和纹理的绑定，并进行后续的深度测试或阴影计算操作。

注意，在创建和使用深度立方体贴图时，需要考虑其特殊的格式和尺寸要求，同时也需要合适的硬件和驱动支持。
...
![img](https://learnopengl-cn.github.io/img/04/06/cubemaps_morrowind.jpg)
...
![img](https://learnopengl-cn.github.io/img/04/06/cubemaps_skybox_result.png)
...
我们根据观察方向向量$\vec{I}$和物体的法向量$\vec{N}$，来计算反射向量$\vec{R}$。我们可以使用GLSL内建的reflect函数来计算这个反射向量。最终的$\vec{r}$向量将会作为索引/采样立方体贴图的方向向量，返回环境的颜色值。最终的结果是物体看起来反射了天空盒。
...
![img](https://learnopengl-cn.github.io/img/04/06/cubemaps_reflection.png)
...
![img](https://learnopengl-cn.github.io/img/04/06/cubemaps_refraction.png)
